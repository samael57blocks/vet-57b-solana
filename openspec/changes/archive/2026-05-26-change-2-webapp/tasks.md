...
> **Archived**: 2026-05-26 — All 28/28 tasks complete. See archive-report.md for full traceability.
...
| Chain strategy | approved (3 PRs) |

Decision needed before apply: **Yes (approved)**
Chained PRs recommended: **Yes**
Chain strategy: **approved (3 chained PRs)**
400-line budget risk: **High**
...
- [x] **3.1** Create `petSolanaService.ts` — `getPets`, `registerPet`
...
- [x] **3.2** Create `appointmentSolanaService.ts` — `schedule`, `pay`, `list`
...
- [x] **3.3** Create `checkinSolanaService.ts` — `takePetToVet`
...
- [x] **3.4** Write service tests (mocked program)
...
- [x] **4.1** Create `RegisterPetForm.tsx` component
...
- [x] **4.2** Update `PetOverview.tsx` for on-chain data
...
- [x] **4.3** Update `usePetsOverview.ts` hook for Solana
...
- [x] **4.4** Update `pet.ts` types (add breed, align with on-chain)
...
- [x] **4.5** Update `PetsOverviewView.tsx` — use new hook, remove inline form
...
- [x] **4.6** Update `PetsOverview.tsx` page — pass loading/error states
...
- [x] **4.7** Write component/hook tests
...
- [x] **5.1** Update `medicalAppointment.ts` types to align with on-chain
...
- [x] **5.2** Create `useAppointments.ts` hook
...
- [x] **5.3** Create appointment UI components (list, schedule, pay)
...
- [x] **5.4** Create check-in UI component
...
- [x] **5.5** Fix `appoinments` → `appointments` directory + NavBar hrefs
...
- [x] **5.6** Add appointment routes to `router.tsx`
...
- [x] **6.1** Update `main.tsx` with provider chain
...
- [x] **6.2** Update `App.tsx` — keep `<Outlet>` layout
...
- [x] **6.3** Verify `npm run build` succeeds with 0 TS errors
...
- [x] **6.4** Run `vitest` — all tests pass
  - **Files**: — (verification step)
  - **Depends on**: all previous
  - **Est. lines**: 0
